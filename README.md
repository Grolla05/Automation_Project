...
- [ ] **Adoção de TypeScript**: Migrar a base de `.jsx` para `.tsx` (garantir tipagem estática, proteção contra variáveis nulas e autocompletar).
  > 🤖 **Prompt para IA:** *"Atue como Engenheiro Front-End React Pleno. Preciso migrar os arquivos `.jsx` do meu projeto Vite para TypeScript (`.tsx`). Comece pelos arquivos `api.js` e a interface do usuário em `WelcomeScreen.jsx`. Crie os tipos exatos baseados nas props atuais, configurando `tsconfig.json` e adicionando `@types/react` no `package.json`. Certifique-se de que a transpilação continue fluída."*
- [ ] **Tratamento Global de Erros (Error Boundaries)**: Implementar uma tela elegante de falha no React que substitua a "tela branca" em caso de quebras inesperadas no Frontend ou PyWebView.
  > 🤖 **Prompt para IA:** *"Crie um componente `<ErrorBoundary>` em React (class component obrigatório) para minha aplicação. O design da tela de erro (fallback UI) deve seguir o Apple Style guide com Tailwind e usar um ícone da Lucide-React. Intercepte `componentDidCatch` gerando logs e o utilize como 'wrapper' direto em volta do meu `<main>` em `App.jsx`, capturando qualquer Exception invisível da tela do usuário."*
...
- [ ] **Roteamento Profissional (React Router / TanStack)**: Substituir o estado local de páginas (`currentScreen === 'UPLOAD'`) por rotas reais para proteger o fluxo de *Wizard* e permitir expansão limpa de novas telas.
  > 🤖 **Prompt para IA:** *"Remova minha renderização condicional frágil mapeada no State do componente pai do `App.jsx`. Quero usar a última versão do `react-router-dom` implementando funções loader/actions ou rotas BrowserRouter puras. As telas antigas devem se tornar `/`, `/upload`, `/process` e `/result`. O roteador precisa permitir retrocesso seguro e lidar com ausência de ID da sessão, blindando as URLs da View."*
